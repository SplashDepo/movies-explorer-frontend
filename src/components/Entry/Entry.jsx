...
function Entry({ children, heading, name, btn, btnAriaLabel, onSubmit, onLoad, isValid, error }) {
  ...
  return (
    <div className="entry">
      ...
      <form className="entry__form" name={name} onSubmit={onSubmit} noValidate>
        <fieldset
        ...
        <div className="entry__wrapper-btn">
          <span
            className={`error${((error?.registrationResponse || error?.authorizationResponse) && 'error_visible') ||
              ''
              } error_type_server-response`}>
            {error?.registrationResponse || error?.authorizationResponse}
          </span>
          <button
            className={`entry__btn ${!isValid && 'disabled'}`}
            type="submit"
